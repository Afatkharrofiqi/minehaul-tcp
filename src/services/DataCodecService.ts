...
export class DataCodecService {
  ...
  async update(imei: string, codec: string): Promise<DataCodec> {
    Logger.log(`Updating data in the database for IMEI: ${imei}`);

    // Find the entity instance using the provided IMEI
    const dataCodec = await this.repo.findOne({ where: { imei } });

    if (!dataCodec) {
      Logger.warn(`No record found with IMEI: ${imei}`);
      throw new Error(`No record found with IMEI: ${imei}`);
    }
    ...
    const savedData = await this.repo.save(dataCodec);
    Logger.log(
      `Data successfully updated for IMEI: ${imei} with new codec: ${codec}`
    );
