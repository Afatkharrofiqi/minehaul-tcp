...
import { DataCodecService } from './DataCodecService';
import { SyncDeviceDataService } from './SyncDeviceDataService';

export class TcpService {
  private server: Server;
  private syncServiceData: SyncDeviceDataService;
  private dataCodec: DataCodecService;

  constructor(
    syncServiceData: SyncDeviceDataService,
    dataCodec: DataCodecService
  ) {
    ...
    socket.on('data', async (data) => {
      ...
      try {
        ...
        if (isImeiPacket) {
          ...
          lastId = await this.dataCodec.insert(data.toString('hex'));
        ...
        this.dataCodec.update(lastId, data.toString('hex'));
        ...
        socket.write(Buffer.from('00000002', 'hex'));
